...
case 'get_cashflow_summary':
    ...
    // Gunakan store_id yang sama untuk semua binding :filter_store_id
    // Nilai kosong akan ditangani di Query SQL menggunakan OR :filter_store_id = ''
    ...
    $sql_union = "
        ...
        WHERE
            ...
            AND (t.store_id = :filter_store_id OR :filter_store_id = '') 
        ...
        WHERE
            ...
            AND (store_id = :filter_store_id OR :filter_store_id = '')
    ...
    try {
        ...
        // Binding parameter selalu dilakukan
        ...
        // Kirim data terperinci dan ringkasan per store
        // jsonResponse($success, $message, $data, $summary, $httpCode)
        jsonResponse(true, 'Cash flow data retrieved successfully.', $cashflow_data, array_values($summary_by_store));

    } catch (PDOException $e) {
        // Jika PDO error, berikan pesan error database secara spesifik
